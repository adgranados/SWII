...
public class TestControlArchivo {
    ...
    @Test
    public void girosExplorador() {
        ExploracionMarte exploracionMarte = ExploracionMarte.getInstancia();
        try {
            exploracionMarte.leerMundo();
        } catch (IOException ex) {
            Logger.getLogger(TestControlArchivo.class.getName()).log(Level.SEVERE, null, ex);
        }
        Explorador explorador = (Explorador) exploracionMarte.getExplorador(1);
        explorador.girarDerecha();

        Assert.assertEquals(explorador.getDireccion(), Explorador.DIRECCION_ESTE);
